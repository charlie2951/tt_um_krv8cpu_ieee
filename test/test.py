...
async def test_cpu_signed_operations(dut):
    ...
    async def run_instr(instr_hex, label):
        ...
        acc_raw = dut.user_project.my_cpu.acc.value
        acc_dec = acc_raw.signed_integer
        x1_val  = dut.user_project.my_cpu.registers[1].value
    ...
    assert dut.user_project.my_cpu.acc.value == 1
